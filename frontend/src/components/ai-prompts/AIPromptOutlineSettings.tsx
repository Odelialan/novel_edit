...
import { Card, Button, Space, message, Typography, Breadcrumb, Tabs, Input, Modal, Form, Select } from 'antd'
...
interface AIPromptOutlineSettingsProps {
  novelId?: string
  onBack?: () => void
}

export default function AIPromptOutlineSettings({ novelId, onBack }: AIPromptOutlineSettingsProps) {
  ...
  const loadOutlinePrompts = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (result.ok) {
          setOutlinePrompts(result.data?.prompts?.outline || {})
        }
      }
    } catch (error) {
  ...
  const saveOutlinePrompt = async (key: string, type: string, value: string) => {
    try {
      const scope = novelId ? 'novel' : 'global'
      const url = novelId 
        ? `/api/utils/prompts?scope=${scope}&novel_id=${encodeURIComponent(novelId)}`
        ...
        <div key={`${key}-${type}`} style={{ marginBottom: 16, padding: 16, border: '1px solid #d9d9d9', borderRadius: 6 }}>
          ...
          <Space>
            <Button 
              type="primary" 
              icon={<SaveOutlined />} 
              onClick={() => saveOutlinePrompt(key, type, editingPrompt.value)}
            >
              保存
            </Button>
  ...
  const renderOutlineTab = (outlineType: string) => {
    const outlineData = outlinePrompts[outlineType]
    if (!outlineData) return <Text type="secondary">暂无数据</Text>
    
    return (
      <div>
        <Title level={4}>{outlineTypes.find(t => t.key === outlineType)?.label}提示词</Title>
        {renderPromptEditor(outlineType, 'random', outlineData.random || '')}
        {renderPromptEditor(outlineType, 'targeted', outlineData.targeted || '')}
      </div>
    )
  }
    ...
    >
      <Breadcrumb 
        ...
        items={[
          { title: 'AI提示词管理' },
          { title: '大纲生成' },
          { title: outlineTypes.find(t => t.key === activeTab)?.label }
        ]}
      />

      <div style={{ marginBottom: 16 }}>
        <Text type="secondary">
          {novelId ? '小说级大纲提示词（仅当前小说可见）' : '全局大纲提示词（所有小说可见）'}
        </Text>
      </div>
      ...
      <Tabs 
        ...
        items={outlineTypes.map(type => ({
          key: type.key,
          label: type.label,
          children: renderOutlineTab(type.key)
        }))}
      />
