...
export default function AIPromptPolishSettings() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.ok) {
          ...
          form.setFieldsValue({
            polish_sentence: data.polish?.sentence,
            polish_paragraph: data.polish?.paragraph,
          })
        }
      }
  ...
  const save = async () => {
    try {
      ...
      const structured = { polish: { sentence: values.polish_sentence, paragraph: values.polish_paragraph } }
  ...
  return (
    <Card
      title={<span><SettingOutlined className="mr-2" />AI 提示词 · 润色</span>}
      extra={
        <Space>
    ...
    >
      <Form form={form} layout="vertical">
        <Form.Item name="polish_sentence" label="句子润色">
          <Input.TextArea autoSize={{ minRows: 12, maxRows: 60 }} />
        </Form.Item>
        <Form.Item name="polish_paragraph" label="段落润色">
          <Input.TextArea autoSize={{ minRows: 12, maxRows: 60 }} />
        </Form.Item>
      </Form>
    </Card>
