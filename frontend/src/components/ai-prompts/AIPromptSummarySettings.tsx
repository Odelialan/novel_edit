...
export default function AIPromptSummarySettings() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.ok) {
          ...
          form.setFieldsValue({ summary_chapter: data.summary?.chapter })
        }
      }
  ...
  const save = async () => {
    try {
      ...
      const structured = { summary: { chapter: values.summary_chapter } }
  ...
  return (
    <Card
      title={<span><SettingOutlined className="mr-2" />AI 提示词 · 概要</span>}
      extra={
        <Space>
    ...
    >
      <Form form={form} layout="vertical">
        <Form.Item name="summary_chapter" label="章节概要">
